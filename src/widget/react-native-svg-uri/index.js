...
class SvgUri extends Component{
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    let ret = {};
    if (nextProps.source) {
      ret.source = nextProps.source;
    }
    if (nextProps.svgXmlData !== prevState.svgXmlData) {
      ret.svgXmlData = nextProps.svgXmlData;
    }
    if (nextProps.fill !== prevState.fill) {
      ret.fill = nextProps.fill;
    }
  ...
  componentDidMount(prevProps, prevState, snapshot): void {
    if (prevProps && prevProps.source && this.props.source.uri !== prevProps.source.uri) {
      this.fetchSVGData(source.uri);
    }
  }
